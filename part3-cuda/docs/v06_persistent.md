...
源码：[v06_persistent.cu](../kernels/v06_persistent.cu) · [交互逐行讲解](v06_persistent.html)
...
先读：[上一版](v05_double_buffer.html) · [下一版](v07_warp_specialized.html)。本版重点：**CTA0：ti0 → ti148（148SM例子）**。下方保留完整逐行代码，先用图和具体例子建立整体过程，再对照行号。
...
![本版完整过程](v06_persistent-process.svg)
...
![本版关键机制](v06_persistent-mechanism.svg)
...
![06 · 持久化 tile 调度](v06_persistent.svg)

## 编译运行

```bash
./build.sh v06_persistent
./build/v06_persistent 4096 4096 4096
```
...
|行|原始代码|解释|
|---|---|---|
|1|`// CUDA C++ / CuTe — 原书第 6 步。`|说明性注释，不生成机器指令。对应的中文机制解释见本节开头和下面的实际语句。|
...
|44|`int step = 6 >= 6 ? gridDim.x : tiles_m * tiles_n;`|持久化模式中每轮任务号加gridDim.x；普通多CTA模式只有一个任务。|
...
|51|`if constexpr (6 >= 6)`|编译期分支：常量条件决定保留哪条路径，未选中的单/双CTA或单/双buffer路径不会在运行时执行。|
...
|183|`int grid = 6 >= 6 ? std::min(tiles, p.sms) : tiles;`|限制持久化并行任务数不超过可用SM/cluster数，也不超过总tile数。更多工作由同一个CTA/cluster在循环里继续处理。|
...
|190|`constexpr int VERSION = 6;`|测试程序用这个编译期版本号检查允许的输入形状，并标记输出JSON。|
...
```json
{
  ...
  "median_ms": 0.330460817,
  "p10_ms": 0.329456002,
  "p90_ms": 0.338659197,
  "tflops": 415.900907,
  "cublas_ms": 0.0825776011,
  "cublasLt_ms": 0.102939203,
  "speedup_vs_lt": 0.31150198,
  "lt_candidates": 8,
  "lt_choice": 0,
