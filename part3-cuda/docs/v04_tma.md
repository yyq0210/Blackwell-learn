...
先读：[上一版](v03_multi_cta.html) · [下一版](v05_double_buffer.html)。本版重点：**TMA输入 + 分片TMA输出**。下方保留完整逐行代码，先用图和具体例子建立整体过程，再对照行号。
...
|每CTA输入/输出SMEM数据|输入32KiB；输出缓冲16KiB；barrier、字段及对齐另计|
|每CTA TMEM|128列，即128×128个32-bit单元|
|教学例子的M×N×K|256×384×192；不改变下方已有实测尺寸与数据|

![本版完整过程](v04_tma-process.svg)
