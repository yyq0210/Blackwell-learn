...
template<int G,int C,int Stages>struct WSConfig{
 static constexpr int BM=128*G,BN=128*G,Threads=128*C+64,Cols=BN*C;
...
__global__ void ws_kernel(TA a,TB b,TD d,CUTE_GRID_CONSTANT CA const ca,CUTE_GRID_CONSTANT CB const cb,CUTE_GRID_CONSTANT CD const cd,int mt,int nt){
 ...
 if(threadIdx.x==0){
  for(int i=0;i<S;++i){initialize_barrier(s.full[i],1);initialize_barrier(s.empty[i],1);}
 ...
 // One leader MMA warp schedules C independent accumulator tiles.
 if(warp==4*C+1&&leader){
  ...
  for(int ti=cluster;ti<mt*nt;ti+=cluster_count,++round){
   ...
   for(int kt=0;kt<size<1>(a)/64;++kt,++iteration){
    ...
    CUTE_UNROLL
    for(int c=0;c<C;++c){
     if(kt==0&&round)wait_barrier(s.acc_empty[c],(round-1)&1);
     ...
     mma.accumulate_=kt==0?UMMA::ScaleOut::Zero:UMMA::ScaleOut::One;
     CUTE_UNROLL
     for(int kb=0;kb<size<2>(ra);++kb){gemm(mma,ra(_,_,kb),rb(_,_,kb),acc);mma.accumulate_=UMMA::ScaleOut::One;}
     if(kt+1==size<1>(a)/64)mma_arrive<G>(&s.acc_full[c]);
    }
